<div class="space-y-4">
    ...
    <div>
        <div class="flex items-center justify-between">
            <span class="text-xs font-semibold uppercase tracking-widest text-ink/60">Steps</span>
            <button type="button" wire:click="addItem('items')" class="text-xs font-semibold text-brand-600 hover:text-brand-700">+ Add step</button>
        </div>
        <div class="mt-2 space-y-3">
            @foreach (data_get($editingContent, 'items', []) as $i => $item)
                <div wire:key="step-{{ $i }}" class="rounded-lg border border-black/10 p-3 space-y-2">
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-semibold text-ink/50">Step {{ $i + 1 }}</span>
                    ...
                    <textarea wire:model="editingContent.items.{{ $i }}.text" placeholder="Description" rows="2" class="block w-full rounded-lg border-black/15 px-3 py-2 border text-sm"></textarea>
                </div>
            @endforeach
        </div>
    </div>
